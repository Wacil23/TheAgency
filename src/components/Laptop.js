...
const Laptop = ({ portal, getPositionScrollFromChild, ...props }) => {
    ...
    const { scene, nodes } = useGLTF('https://vazxmixjsiawhamofees.supabase.co/storage/v1/object/public/models/macbook/model.gltf')

    useFrame((state, delta) => {
        getPositionScrollFromChild(window.scrollY)
        const r1 = scroll.range(0 / 4, 1 / 4)
        const r2 = scroll.range(2 / 4, 2 / 4)
        nodes.Top.rotation.x = Math.PI - (Math.PI / 2) * rsqw(r1) + r2 / 12
    ...
    return (
        ...
        <>
            ...
            <PresentationControls polar={[-0.2, 0.03]} azimuth={[-1, 0.75]} config={{ mass: 2, tension: 400 }} >
                <primitive position-y={-2} object={scene} {...props}>
                    <Html transform wrapperClass='frame' position={[0, 1.56, -1.4]} distanceFactor={1.17} >
                        <iframe className='' src="https://yummy-wok.fr" title='web'>
                            <h1>Hello dfsdfsdf</h1>
                        </iframe>
                    </Html>
                </primitive>
            </PresentationControls>
        ...
        //         <Html position-y={-3}  portal={portal} >
        //             <iframe className='' src="https://yummy-wok.fr" title='web'>
        //                 <h1>Hello dfsdfsdf</h1>
        //             </iframe>
        //         </Html>
